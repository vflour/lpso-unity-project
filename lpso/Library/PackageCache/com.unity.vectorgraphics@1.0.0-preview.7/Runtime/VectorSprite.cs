...
namespace Unity.VectorGraphics
{
    public static partial class VectorUtils
    {
        ...
        public static Sprite BuildSprite(List<Geometry> geoms, float svgPixelsPerUnit, Alignment alignment, Vector2 customPivot, UInt16 gradientResolution, bool flipYAxis = false)
        {
            ...
            if (colors != null)
            {
                ...
                using (var nativeColors = new NativeArray<Color32>(colors32.ToArray(), Allocator.Temp))
                    sprite.SetVertexAttribute<Color32>(UnityEngine.Rendering.VertexAttribute.Color, nativeColors);
            }
            if (uvs != null)
            {
                using (var nativeUVs = new NativeArray<Vector2>(uvs.ToArray(), Allocator.Temp))
                    sprite.SetVertexAttribute<Vector2>(UnityEngine.Rendering.VertexAttribute.TexCoord0, nativeUVs);
                using (var nativeSettingIndices = new NativeArray<Vector2>(settingIndices.ToArray(), Allocator.Temp))
                    sprite.SetVertexAttribute<Vector2>(UnityEngine.Rendering.VertexAttribute.TexCoord2, nativeSettingIndices);
            }
        ...
        public static void RenderSprite(Sprite sprite, Material mat) 
        {
            ...
            if (sprite.HasVertexAttribute(UnityEngine.Rendering.VertexAttribute.Color))
                colors = sprite.GetVertexAttribute<Color32>(UnityEngine.Rendering.VertexAttribute.Color);

            NativeSlice<Vector2>? settings = null;
            if (sprite.HasVertexAttribute(UnityEngine.Rendering.VertexAttribute.TexCoord2))
                settings = sprite.GetVertexAttribute<Vector2>(UnityEngine.Rendering.VertexAttribute.TexCoord2);
